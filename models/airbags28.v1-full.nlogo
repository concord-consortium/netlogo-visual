...
; to turn ON automatic graphing in the second graph when a slider is moved go to the first line in initialize-author-tools  
; and set auto-p-graph? true. This allows a run that was automatically generated by moving a slider to place a point in grid2. 

__includes [ "data-export-modular.nls" ] ;;JVREINSTATED ; used in exporting data

globals [
  version-number
  continuous-graph?        ; determines whether the time-series is updated continuously as the sliders are changed. 
  instant?                 ; a flag indicating that the current run is generated by a slider change.
  slider                   ; stores the slider that initiated the instant run. 0=car speed, 1=distance, 2=size, 3=time-to-fill
  auto-p-graph?            ; determinse whether automatically-generated runs generate a point in p-space 
  what-is-your-goal?
  ; author globals
  ...
  i-data          ; saves data from instant runs
...
to initialize
  ...
  set version-number 28.1
  set continuous-graph? true         ; ensures that the graph is changed every time a parameter is changed
;  output-print (word "Run Survives   MA    Car speed   Distance  Bag size  Fill time")
  ...
  set run-data (list version-number)      ; this global stores the results of runs. Each run creates a list of data 
  set i-data []

  setup-data-export ;;JVREINSTATED    ;;; initialize the modular JSON data export functions
...
to on/off    ; this is the main execution loop
  if starting-up? [
      ; executed only once the first time the on/off button is pressed
...
to act-on-changes   ; look for changes in the sliders and selectors and takes appropriate action 
  ...
  if omit-question? [set variables-locked? false]    ; 
  ...
  if (old-vertical-axis != vertical-axis) [
    ...
    draw-view   ; 
    ;;JVREMOVED set activity-counter activity-counter + 1 
    update-run-data ;;JVREINSTATED
    ]
  ...
  if car-speed != old-car-speed [  ; if the variables are locked, don't let the user change this variable
    if continuous-graph? [
      set slider 0       ; remember that slider 1 initiated this run
      run-instant-airbag]
    ifelse variables-locked? 
      [set car-speed old-car-speed]
      [set old-car-speed car-speed ]]
  ...
  if old-distance-to-steering-wheel != distance-to-steering-wheel [   ; indicates that this slider has been changed
    ifelse variables-locked?          ; if the variables are locked... 
       ...
       [set old-distance-to-steering-wheel distance-to-steering-wheel ; if not locked...
         ifelse continuous-graph? 
           [set slider 1 run-instant-airbag]            ; remember that slider 1 initiated this run
  ...
  if old-airbag-size != airbag-size [
    ifelse variables-locked? 
      [set airbag-size old-airbag-size]
      [set old-airbag-size airbag-size 
        if continuous-graph? [
          run-instant-airbag
          set slider 2]]] 
  ...
  if old-time-to-fill-bag != time-to-fill-bag [
     ifelse variables-locked? 
       [set time-to-fill-bag old-time-to-fill-bag]
       [set old-time-to-fill-bag time-to-fill-bag
          if continuous-graph? [
            run-instant-airbag
            set slider 3]]] 
  ...
  if old-pick-y-axis != pick-y-axis and run-number > 0 [
...
to run-instant-airbag   ; special way to get to run-airbag when the run is initiated by a slider change
  set instant? true     ; used to determine what run data to save
  run-airbag
end
...
to support-mouse      ; first ask whether the mouse is in one of the grids
  ...
  if in-grid? mouse-xcor mouse-ycor [  ; if in the main grid and there is only one graph, support a cursor
    ...
    if n-graphs = 1 [   ; if exactly one graph is showing and its number is graph-number
      ...
      if not was-in-grid? [      ; get here only when entering the grid--used for research data collection
        ...
        ;;JVREMOVED set activity-counter activity-counter + 1    ; save the fact that the cursor entered the grid
        update-run-data ;;JVREINSTATED
  ...
  if continuous-graph? and auto-p-graph? [set ig2? false ]  ; do not support the cursor in graph2 if points are added to graph2 
                                                             ;     automatically as parameters are being changed
...
; the right-hand graph is number two. 
...
to run-airbag ; computes and draws the position or velocity graphs of the airbag and dummy 
  ...
  if run-number > 0 [update-run-data] ;;JVREINSTATED
...
;    if not continuous-graph? [update-data-series ( list t x-dum v-dum )]
    update-data-series ( list t x-dum v-dum ) ;;JVREINSTATED
    ...
    if t = 0 and not continuous-graph? [ tick wait .5]                     ; pause once the initial location is shown
    set t t + dt 
    if not continuous-graph? [ tick ] ]                                    ; update the screen. This is the end of one time step of dt
  ...
  show-results      ; updates the parameter graph for this run
  
  ; package up the results of this run and information about what the student did prior to this run
  ; it may seem odd to place this here, but it is only here that the code can know that the student is
                    ...
                    ;       has done after the previous run but before the one just completed.... 
  if not instant? [ ; the packaging is done only after a run initiated by a student pressing the RUN button 
                    ; instant runs are treated differently. data from them are maintined in i-data and incorporated in 
                    ;       the data stored with the next (non-instant) run
    ;;MOVED 
    ; update temp-data which contains timestamp, four sliders, three outcomes
    ...
    set temp-data lput a-max-g temp-data  ; add on the max acceleration in g units      5 
    set temp-data lput dummy-status-number temp-data                       ;                   6
    set temp-data lput logic-number dummy-crashed? temp-data                           ;7
    
    update-run-series ;;JVREINSTATED-MOVED (above update-run-data)   
    
    ;;set run-data lput (list temp-data tool-data i-data) run-data ; create and save the complete record for this run
                ; note tool-data is a reporter that returns a list 
    update-run-data ;;JVREINSTATED
    
    reset-timer ; begin timing how long the student analyzes data for the next run  
    set i-data []     ; clear out these in preparation for the next run  
               
  ]
  if instant? [   ; reset updata i-data for this instant run (the run was generated by moving a slider and the following are saved
    ; note that these refer to the slider values that initiated the current instant run. 
    ;   0. time since the end of the last run
    ;   1. slider changed. 0=car speed, 1=distance, 2=size, 3=time-to-fill
    ;   2. new slider value 
    ;   3. a-max-g  (maximum acceleration in g units)
    let val 0
    if slider = 0 [set val car-speed]        ; slider is set in act-on-changes when user changes a slider
    if slider = 1 [set val distance-to-steering-wheel]
    if slider = 2 [set val airbag-size]
    if slider = 3 [set val time-to-fill-bag]
    let one-i-record (list timer slider val a-max-g)
    set i-data lput one-i-record i-data   ; append a list containing data for this run to i-data
  ]
end
...
to-report dummy-status-str [n]  ; converts dummy status to a string ;;JVREINSTATED
  if n = 0 [report "Yes"]
  if n = 1 [report "No"]
  if n = 2 [report "Maybe"]
end
...
to setup-for-run
  if run-number > 0  [update-run-data] ;;JVREINSTATED
  if continuous-graph? [ask dots [ht]
    ask dots with [run-n = 0 ][die]]   ; dots on the instant graphs are not retained
  if not continuous-graph? [
    set run-number run-number + 1 ] ; increment the run counter   
  if always-erase? [                ; the dots are not erased, just hidden
    ask dots [ht]     ; remove all prior graphs 
    ask lines [set hidden? true]
    set pick-graphs "Last 1"
    set old-pick-graphs "Last 1"]
  ask messages [ht]
  let d-color cyan; the dummy and position and velocity lines are all cyan
  ask actors   [st]
  set variables-locked? not continuous-graph?  ; used to stop any changes during a run
;  set pick-y-axis "Position (m)"
  set old-pick-y-axis pick-y-axis
  set-y-axis 
  if not continuous-graph? [tick] 
  
  set-times          ; computes   t-start-inflate The time at which the airbag starts inflating
                     ;            t-fully-inflated, the time at which the airbag is fully inflated
                     ;            t-car-stops, the time at which the car stops
                     
  ; there are a bunch of variables used in student logging that need to be initialized. Reported after each run in tool-data
  set run-groups-used []
...
to recolor-dummy-graph   ; colors graph run-number red, yellow, or green depending on a-max
  ; also generates a star at a-max
  let c maybe-color
  if a-max-g < .8 * a-cutoff  [set c safe-color]  ; give a 20% margin 
  if a-max-g > 1.2 * a-cutoff  [set c unsafe-color]
  ; c is now the right color
  let max-list []       ; sometimes multiple points are at the maximum acceleration. This will hold the who of each of them
  ask dots with [color = cyan ][  ; select current bag points
    set color c                                          ; recolor the dots
    ; now find the dots that are near the maximum acceleration    ; the real acceleration of this dot in mks units
    if (acceleration = a-max) [            ; if this dot is at the max acceleration, save its who
      set max-list lput who max-list ]]                 ; add a point to a list of whos of points with max acceleration
  ; now pick one of max-list to show a star representing the max acceleration
  
  if length max-list != 0 [        ; for safety. max-list should always contain at least one entry.
    let w-max one-of max-list 
    ask dots with [who = w-max][
      set shape "star" set size 3]]      ; mark the maximum
  ask lines with [color = cyan][ set color c ]  ; recolor the lines
end
...
to update-dummy [t x-dum v-dum acc d-color]  ; places a point on the graph and puts the dummy on the stage
  ...
  create-dots 1 [          ; create a dummy graph dot for time=t
    ...
    ifelse continuous-graph?  
      [set run-n 0 ]   ; all runs generated by changing the sliders are called run 0
    ...
    ifelse w > 0 [                ; connect the new dot with the prior one using a colored line, except for the first dot
      ...
      create-line-with dot w [
        set thickness normal-thickness
         ifelse continuous-graph?  
           [set run-num 0 ]   ; all runs generated by changing the sliders are called run 0
...
to display-runs-colored [clr]           ; displays only runs that have the color clr
  let i 1
  while [i <= run-number  and i > 0][             ; go through all runs
...
to update-run-data ;;JVREINSTATED
  ...
  let current-run-data (list temp-data tool-data i-data)                                  ; temp-data stores the data on sliders etc. used in the run
...
;  show activity-counter
  ;;;FROM ABOVE set run-data lput (list temp-data tool-data i-data) run-data ; create and save the complete record for this run      
...
to-report tool-data 
  ; tool-data records student use of analysis tools and returns a list of the following 
  ;   0. prior-runs-viewed (a list of run numbers viewed)
  ;   1. graph-type-used (a list consisting of 1s and 2s, 1 for position, 2 for velocity)
  ;   2. run-groups-used ( a list of the texts selected using the pick-graph function)
  ;   3. used-cursor? (true false) true if the mouse ever entered the time-series graph
  ...
  ;   9. hover-times  The number of times that the user hovered over a dot in the parameter graph area
  ;  10. activity-counter. An overall measure of student interaction--the number of actions a student takes
  let td list prior-runs-viewed graph-type-used             ; 0, 1
  set td lput run-groups-used td            ; 2
  set td lput logic-number used-cursor? td  ; 3
  set td lput cursor-times td               ; 4
  set td lput round cursor-time td          ; 5
  set td lput logic-number used-pointer? td ; 6
  set td lput pointer-times td              ; 7
  set td lput round pointer-time td         ; 8
  set td lput hover-times td                ; 9
  set td lput activity-counter td           ; 10
  set prior-runs-viewed []
  set graph-type-used []
  set run-groups-used []
  set used-cursor? false
  set cursor-times 0
  set cursor-time 0
  set used-pointer? false
  set pointer-times 0
  set pointer-time 0
  set hover-times 0
  set activity-counter 0
...
to show-results       ; generates a line of text in the output box and point in parameter space summarizing a run
  ...
  if a-max-g < .8 * a-cutoff  [set dummy-status "Yes"]  ; give a 20% margin 
  if a-max-g > 1.2 * a-cutoff  [set dummy-status "No"]
  
  ; finally, create one dot2 for the parameter graph
  if continuous-graph? and auto-p-graph? [ask dots2 [set size 3]]   ; make all prior dots smaller
  ; drop a point in p-space unless this is a continuous graph and auto-graphing is no allowed
  if not continuous-graph? or (continuous-graph? and auto-p-graph?) [
  create-dots2 1 [
...
to setup-data-export
  ...
  let model-information [
    [ "airbags" "airbags28.v1.nlogo" "28.v1" ] ]
...
to initialize-author-tools   ; in a previous version, these variables were set in the UI
  ...
  set auto-p-graph? false     ; allows a run that was automatically generated by moving a slider places a point in grid2. 
  set slow-mo? false
  ...
  set grid-v-Color 3         ; the color of the grid in the main graph for the velocity graph
  set back-v-color  48       ; the color of the grid in the main graph for the velocity graph
...
237
Run Model
set continuous-graph? false\nset instant? false\nrun-airbag\nset continuous-graph? true
NIL
1
T
OBSERVER
