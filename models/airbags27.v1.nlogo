...
; Because of the many versions of Airbags and the software from which this was derived, there is a lot of code that is not needed below. 

__includes [ "data-export-modular.nls" ]  ; used in exporting data
...
to-report the-question-number  ; converts the selected question to a number
  if what-is-your-goal? = "Test a minimum of maximum value" [report 1]
  if what-is-your-goal? = "Make a small change from the the last run" [report 2]
  if what-is-your-goal? = "Fill a gap in my results" [report 3]
  if what-is-your-goal? = "Do a controlled comparison" [report 4]
  if what-is-your-goal? = "Explore/other" [report 5]
  report 0
end
...
;  show activity-counter
  ...
  set run-data-word (word fput version-number run-data)
end
...
to initialize-author-tools   ; in a previous version, these variables were set in the UI
                             ;     so that the author could experiment with different configurations. 
  set slow-mo? false
